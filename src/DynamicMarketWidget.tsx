...
import { RWA_MARKET_ABI } from "./blockchain/rwaMarket";
...
const DynamicMarketWidget: React.FC<MarketWidgetProps> = ({ contractAddress, name, symbol }) => {
  ...
  useEffect(() => {
    async function load() {
      if (window.ethereum && contractAddress) {
        ...
        const contract = new ethers.Contract(contractAddress, RWA_MARKET_ABI, provider);
  ...
  const handleBuy = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const contract = new ethers.Contract(contractAddress, RWA_MARKET_ABI, signer);
...
export default DynamicMarketWidget;
